...
function updateTodo() {
  axios
  .patch('https://jsonplaceholder.typicode.com/todos/1',{title:'New Todo',completed:true})
  .then(res=> showOutput(res))
  .catch(err=> showOutput(err))
}
...
function errorHandling() {
  ...
  .get('https://jsonplaceholder.typicode.com/todoss',{
    validateStatus: function(status){
      return status <500; //reject only if status is greater to 500
    }
  })
  .then(res =>showOutput(res))
  .catch(err =>{
      if(err.response)
        //serve responded with a status other than 200 range 
